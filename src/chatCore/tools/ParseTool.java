...
public class ParseTool {
	...
	public static MsgHead parseMsg(byte[] data) throws IOException {
		...
		if (msgtype == 0x01) {// 如果是注册信息
			...
			MsgSignalKeysResp mskr = new MsgSignalKeysResp();
			mskr.setReistrationId(dins.readInt());
			mskr.setPreKeys(dins.readNBytes(33));
			mskr.setIdentityKey(dins.readNBytes(33));
			mskr.setSignedPreKey(dins.readNBytes(33));
			mskr.setSignedPreKeySig(dins.readNBytes(64));
			mskr.setFirend_id(dins.readInt());
			mskr.setType((byte)0x66);
			mskr.setDest(dest);
			mskr.setSrc(src);
			mskr.setTotalLen(totalLen);
			return mskr;
		}
